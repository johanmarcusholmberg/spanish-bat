...
import {
  ...
  Lock, Trophy, ArrowRight, RotateCcw, GraduationCap, Pencil, Star
} from "lucide-react";
...
const GrammarPage = () => {
  ...
  const lessons = useMemo(
    () => getItemsForLevel(grammarLessons, user?.level || "A1"),
    [user?.level]
  );

  const grouped = useMemo(() =>
    lessons.reduce<Record<string, GrammarLesson[]>>((acc, l) => {
  ...
  // Check if lesson is unlocked (first in level always unlocked, rest need previous completed)
  ...
  const checkAnswer = () => {
    ...
    if (correct || attempts >= 1) {
      // Record result on first correct or after 2 attempts
  ...
  const nextExercise = () => {
    ...
    if (nextIdx >= currentLesson.exercises.length) {
      // Calculate score
      const correctCount = exerciseResults.filter(Boolean).length + (showResult && answer.trim().toLowerCase() === currentExercise?.answer.toLowerCase() ? 0 : 0);
      const finalResults = [...exerciseResults];
      // If last wasn't recorded yet
