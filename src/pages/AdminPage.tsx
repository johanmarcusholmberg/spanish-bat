...
const AdminPage = () => {
  ...
  const fetchMessages = useCallback(async () => {
    const { data, error: fnError } = await supabase.functions.invoke("admin-actions", {
      body: { action: "get_contact_messages" },
    });
    if (fnError) throw new Error(fnError.message);
    if (data?.error) throw new Error(data.error);
  ...
  const saveNotes = async (messageId: string) => {
    try {
      const notes = editingNotes[messageId] || "";
      await supabase.functions.invoke("admin-actions", {
        body: { action: "update_contact_status", message_id: messageId, status: messages.find(m => m.id === messageId)?.status || "new", admin_notes: notes },
      });
