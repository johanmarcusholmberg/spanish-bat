...
export const useVocabularySRS = () => {
  const recordReview = useCallback(async (
    ...
    await api.vocabulary.update(wordId, {
      reviewState: review_state,
      nextReview: nextReview.toISOString(),
      easeFactor: ease_factor,
      intervalDays: interval_days,
      reviewCount: currentWord.review_count + 1,
      correctCount: currentWord.correct_count + (isCorrect ? 1 : 0),
      incorrectCount: currentWord.incorrect_count + (isCorrect ? 0 : 1),
      learned: review_state === "mastered",
    }).catch(() => {});
