...
function readField(entry, field) {
    if (field && typeof entry[field] === "string")
        return entry[field];
    if (typeof entry.es === "string")
        return entry.es;
    return entry.id;
}
/**
 ...
 */
function substitute(text, assignments) {
    ...
    return prev.replace(/\{(\w+)(?:\.(\w+))?\}/g, (_m, name, field) => {
 ...
 */
function sampleAssignments(template, banks, maxCombinations, rng) {
    const varNames = Object.keys(template.variables);
    const pools = varNames.map((name) => {
...
function buildOptions(template, answer, banks, rng) {
    ...
    if (template.distractors) {
        ...
        const candidates = bank.entries
            ...
            .filter((v) => v && v !== answer);
        const picked = shuffle(Array.from(new Set(candidates)), rng).slice(0, template.distractors.count);
        if (picked.length === 0)
            return undefined;
...
export function generatePracticeItems(opts = {}) {
    ...
    const allowedLevels = (() => {
        if (opts.levels && opts.levels.length > 0)
            return new Set(opts.levels);
        if (opts.upToLevel) {
            const max = LEVEL_RANK[opts.upToLevel];
            return new Set(Object.keys(LEVEL_RANK).filter((l) => LEVEL_RANK[l] <= max));
        }
        return null;
    })();
    const items = [];
    ...
    for (const template of templates) {
        if (allowedLevels && !allowedLevels.has(template.level))
            continue;
        ...
        for (const assignment of assignments) {
            ...
            if (seenIds.has(id) || excludeIds.has(id))
                continue;
            seenIds.add(id);
 ...
 */
export function toPracticeItem(generated) {
    return {
        id: generated.id,
